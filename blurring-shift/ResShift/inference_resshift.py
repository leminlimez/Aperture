...
# image = modal.Image.debian_slim().from_dockerfile(
#     "./Dockerfile",
#     add_python="3.10"
# ).pip_install("google", "protobuf", "google-api-python-client")
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-i", "--in_path", type=str, default="./app_images/user.jpg", help="Input path.")
...
def get_configs(args):
    ...
    if args.task == 'realsr':
        ...
        ckpt_url = _LINK[args.task]
        ckpt_path = ckpt_dir / f'ema_model_2000.pth'
        # ckpt_path = ckpt_dir / f'resshift_{args.task}_s{_STEP[args.task]}.pth'
        vqgan_url = _LINK['vqgan']
        vqgan_path = ckpt_dir / f'autoencoder_vq_f4.pth'
    else:
    ...
    print(f"\nDiffusion model path: {ckpt_path}\n")
...
def run_inference(image=None, load_model=False, sampler=None, args=None):
    if load_model:
        args = get_parser()
        ...
        return resshift_sampler, args
    else:
        resshift_sampler = sampler
        return resshift_sampler.inference(
                args.in_path,
                args.out_path,
                image=image,
                mask_path=None,
